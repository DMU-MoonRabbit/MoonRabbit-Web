...
export const ConcernContent: React.FC = () => {
  const { concern, setConcern, toggleConcernLike, concerns } =
  ...
  const handleLikeToggle = async () => {
    ...
    try {
      ...
      if (response.data && typeof response.data === 'object') {
        ...
        const newLikeStatus =
          updatedData.likedByMe ?? updatedData.liked ?? !isCurrentlyLiked
      ...
      const fetchConcern = async () => {
        try {
          ...
          try {
            ...
            response = await axios.get(ENDPOINTS.CONCERN_DETAIL(boardId), {
              headers,
              withCredentials: true,
            })
          } catch (authError) {
      ...
      <div className="flex items-center justify-center w-full">
        ...
        <div
          className={clsx(
            ...
            isMobile
              ? 'w-[calc(100%-2rem)] mx-auto p-8 mt-8 mb-12'
              : 'w-4/5 p-[50px] pb-[32px] mx-2 my-24 ',
          )}
        >
          <p className={clsx(isMobile ? 'text-[24px]' : 'text-[30px]')}>
            {title}
          </p>
          <div
            className={clsx('flex items-center', isMobile ? 'my-4' : 'my-5')}
          >
            {/* 프로필 이미지 + 테두리 */}
            <div
              className="relative w-[30px] h-[30px] mr-[12px]"
          ...
          <p
            className={clsx(
              'whitespace-pre-line break-words font-gothicFont',
