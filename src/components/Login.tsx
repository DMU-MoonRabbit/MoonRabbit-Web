  ...
  return res === 'pc' ? (
    <div className='mb-6 flex items-center'>
      ...
      <div className='flex items-end font-bold ml-auto text-[14px] flex-col lg:items-center lg:flex-row'>
        <span onClick={() => setIsLogin(true)} className={`cursor-pointer ${hoverText}`}>ID&PW 로그인</span>
        <div className='flex items-center'>
          <div className='h-[25px] w-[1px] bg-black mx-2 opacity-0 lg:opacity-100' />
          <span onClick={() => setIsLogin(false)} className={`cursor-pointer ${hoverText}`}>회원가입</span>
        </div>
      </div>
    </div>
  ) : (
